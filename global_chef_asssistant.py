...
/* ---- Hero banner ---- */
.hero {
    background: linear-gradient(135deg, var(--clay) 0%, #8B3A1A 55%, var(--bark) 100%);
    border-radius: 20px;
...
/* ---- Button ---- */
.stButton > button {
    background: linear-gradient(135deg, var(--clay), #A0522D) !important;
...
<div class="hero">
    ...
    <h1>Mama Cooks</h1>
    <p>Your personal Nigerian kitchen guide — from Abuja to Aba, one recipe at a time.</p>
</div>
""", unsafe_allow_html=True)
...
if suggest:
    if ingredients.strip():
        ...
        with st.spinner("Mama Cooks is thinking… 🔥"):
        ...
        <div class="recipe-box">
            <div class="recipe-header">🍽️ Mama Cooks Says…</div>
            <div class="recipe-content">{recipe_text}</div>
        </div>
        """, unsafe_allow_html=True)

    else:
        st.warning("⚠️ Please enter some ingredients before asking Mama Cooks!")

# ── Footer ────────────────────────────────────────────────────────────────────
st.markdown("<hr>", unsafe_allow_html=True)
st.markdown(
    "<p style='text-align:center; color:#9B8070; font-size:0.82rem; font-style:italic;'>"
    "Made with ❤️ & palm oil · Powered by Groq</p>",
    unsafe_allow_html=True
)